...
func (c *VerifyCommand) Exec(ctx context.Context, images []string) (err error) {
	...
	case keyRef != "":
		...
		fmt.Println("\nsigs.PublicKeyFromKeyRefWithHashAlgo(ctx, keyRef, c.HashAlgorithm)", "ctx:", ctx, "keyRef:", keyRef, "c.HashAlgorithm:", c.HashAlgorithm, "result:", pubKey)
	...
	for _, img := range images {
		if c.LocalImage {
			...
			fmt.Println("verify.go 177:", verified)
			if err != nil {
				return err
			}
